...
void Jeffry::ApplyDamage(Creature* attacker, const DamageInfo& info)
{
	...
	else if (info.element == Element::NONE && info.type == DamageType::RANGED) {
		std::cout << "Ranged attack! Crap that is super effective!";
		DuplacateInfo.amount *= 3;
		Creature::ApplyDamage(attacker, DuplacateInfo);
	}
